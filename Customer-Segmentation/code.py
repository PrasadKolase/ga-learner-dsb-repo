...
# Merge dataframes
df = pd.merge(left=offers, right=transactions, left_on='Offer #', right_on='Offer #')
...
cluster_champagne = list(champagne.keys())[list(champagne.values()).index(max(champagne.values()))]
...
for i in range(5):
    ...
    counts = sum(new_df['Discount (%)'])/len(new_df['Discount (%)'])
